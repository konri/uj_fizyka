...
namespace bill {
    ...
    using BillIntegrator = std::function<std::pair<vector, vector>(std::pair<vector, vector>,
                                         std::pair<vector, vector>, vector, double)>;
    ...
    public:
        BillMaterialPoint(BillIntegrator algorithm, vector position = vector({0., 0., 0.}),
                          vector velocity = vector({0., 0., 0.}),
                          double mass = 1.0, vector color = vector({1.0, 0.0, 0.0}), double step = 0.2);
...
// disable/enable simulation
    ...
    public:
        Obstacle() {}
        virtual ~Obstacle();
        virtual bool handleCollision(std::shared_ptr<bill::BillMaterialPoint> &mp) = 0;
    ...
    public:
        ...
        void AddObstacle(std::shared_ptr<Obstacle> p);
