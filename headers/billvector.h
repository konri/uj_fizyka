...
namespace bill {
    ...
    public:
        ...
        vectorNd<T, N>(std::initializer_list<T> list) {
            if (list.size() != N) {
                std::cerr << ("bill::vector<T,N>: initialization list is not the size of vector!"
                        " Additional numbers are removed!\n");
            }
        ...
        vectorNd<T, N>(std::vector<T> vec) {
            if (vec.size() != N)
                std::cerr
                        << ("bill::vector<T,N>: initialization vector is not the size of vector!"
                                " Additional numbers are removed!\n");
    ...
    public:
        ...
        quaternion(std::initializer_list<double> list) {
            ...
            if (list.size() > 4) {
                std::cerr << ("bill::quaternion: initialization list is not the size of vector!"
                        " Additional numbers are removed!\n");
                std::vector<double> tmp;
        ...
        quaternion(double valueS, std::initializer_list<double> list) : s(valueS) {
            ...
            if (list.size() > 3) {
                std::cerr
                        << ("bill::quaternion: initialization vec is not the size of vector! "
                                "Additional numbers are removed!\n");
                v = vector(list);
            }
        }
        ...
        quaternion(std::vector<double> vec) {
            ...
            if (vec.size() > 4) {
                std::cerr << ("bill::quaternion: initialization vec is not the size of vector! "
                        "Additional numbers are removed!\n");
                s = vec[0];        // first element is scalar
                v = vector(vec);
            }
